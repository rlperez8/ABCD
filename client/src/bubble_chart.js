...
export default function BinBubbleChart({ data = [] }) {
  ...
  // Normalize bubble size (so they don’t get huge)
    ...
    datasets: types.map((type, idx) => ({
      ...
      data: data
        ...
        .map(d => ({
          ...
          r: Math.max(5, (d.count / maxCount) * 20), // bubble size
          bin: d.bin,
          avg_return: d.avg_return,
          win_rate: d.win_rate,
          count: d.count,
        })),
      backgroundColor: `hsla(${(idx * 60) % 360}, 70%, 60%, 0.6)`,
