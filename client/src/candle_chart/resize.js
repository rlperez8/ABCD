...
export const push_price_to_middle_screen = (candleChartRef, rust_pattern) => {
    ...
    const half_screen_height = candleChartRef.current.height.startingBaselineY/ 10 * 3.5;
...
export const reposition_candles = (candleChartRef, rust_pattern) => {
    ...
    while (loop === true) {
        ...
        // too large → zoom out (increase size)
        if (price_pixel_location > pattern_box_height) {
            size *= 2
        }
        // too small → zoom in (decrease size)
        else if (price_pixel_location < pattern_box_height / 2) {
            size /= 2
        }
        // fits nicely → finalize
        else {
        ...
        // Safety break in case of extreme edge cases
        if (size > 1e6 || size < 1e-6) {
            console.warn("Loop break: unable to normalize pattern size.")
            break
        }
