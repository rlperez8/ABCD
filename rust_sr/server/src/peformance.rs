...
impl MonthlySummary {
    ...
    pub fn from_grouped_patterns(
        grouped_by_month: &std::collections::HashMap<u32, Vec<Pattern>>,
    ) -> Vec<Self> {
...
impl YearlySummary {
    ...
    pub fn get_yearly_summary(grouped_by_year: &HashMap<i32, Vec<Pattern>>) -> Vec<Self> {
        ...
        for (year, patterns) in grouped_by_year {
            let closed: Vec<&Pattern> = patterns.iter().filter(|p| p.trade_open == false).collect();

            let total = closed.len();
            let wins = closed.iter().filter(|p| p.trade_result == 1).count();
            let win_pct = MonthlySummary::calc_win_pct(total, wins);

            yearly_summary.push(YearlySummary::new(
                *year,
                total as u32,
                wins as u32,
                win_pct,
