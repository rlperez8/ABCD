#!/usr/bin/env python3
"""Train a LightGBM Level 4 manager over LightGBM L1/L2/L3 specialist scores."""
...
DEFAULT_LEVEL1_PREFIX = "aicw-oracle-start-lightgbm-tfspec-v1"
DEFAULT_LEVEL2_RUN = "aicw-oracle-start-lightgbm-root-alltf-v1-NQ-alltf-tr2024-v2026"
DEFAULT_LEVEL3_RUN = "aicw-oracle-start-lightgbm-global-alltf-v1-NQ-alltf-tr2024-v2026"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-prefix", default="aicw-oracle-start-lightgbm-level4-manager-v1")
...
def load_light_model(run_id_value: str):
    return lgbm_utils.load_model(wave.ABCD_ROOT / "model_registry" / run_id_value)


def score_with_light(frame: pd.DataFrame, run_id_value: str) -> np.ndarray:
    model, cat_features, num_features, category_maps = load_light_model(run_id_value)
    return lgbm_utils.predict_scores(frame, model, cat_features, num_features, category_maps)
...
def score_lower_levels(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for timeframe in parse_timeframes(args.timeframes):
        ...
        if part.empty:
            continue
        part["l1_score"] = score_with_light(part, level1_run_id(args, timeframe))
    ...
    work["l2_score"] = score_with_light(work, args.level2_run_id)
    work["l3_score"] = score_with_light(work, args.level3_run_id)
...
def add_manager_scores(frame: pd.DataFrame, model_bundle) -> pd.DataFrame:
    model, category_maps = model_bundle
    scored = frame.copy()
    scored["manager_score"] = lgbm_utils.predict_scores(scored, model, CAT_FEATURES, NUM_FEATURES, category_maps)
    return scored
...
def write_readable(output_dir: Path, train_summary: dict[str, Any], valid_summary: dict[str, Any], args: argparse.Namespace) -> None:
    rows = [
        readable_row(str(args.train_year), train_summary),
        readable_row(str(args.valid_year), valid_summary),
    ]
    with (output_dir / "lightgbm_level4_readable_summary.csv").open("w", newline="", encoding="utf-8") as handle:
...
def main() -> int:
    ...
    print(f"Building LightGBM lower-level reports for train year={args.train_year}", flush=True)
    train_base, train_summary = build_base_rows(int(args.train_year), args)
    train_reports = score_lower_levels(train_base, args)
    print(f"Building LightGBM lower-level reports for validation year={args.valid_year}", flush=True)
    valid_base, valid_summary = build_base_rows(int(args.valid_year), args)
    valid_reports = score_lower_levels(valid_base, args)

    train_for_model = cap_train_rows(train_reports, args)
    print(
        f"Training LightGBM Level 4 manager rows={len(train_for_model):,}/{len(train_reports):,} "
        f"positives={int(train_for_model['is_oracle_start'].sum()):,}",
        flush=True,
    )
    model, category_maps = lgbm_utils.train_binary(train_for_model, args, CAT_FEATURES, NUM_FEATURES)
    train_scored = add_manager_scores(train_reports, (model, category_maps))
    valid_scored = add_manager_scores(valid_reports, (model, category_maps))
    train_scored["scored_rows_marker"] = len(train_scored)
    valid_scored["scored_rows_marker"] = len(valid_scored)
    ...
    for row in train_sweep:
        row["scored_rows"] = int(len(train_scored))
    ...
    for row in valid_sweep:
        row["scored_rows"] = int(len(valid_scored))
    ...
    selected_train["scored_rows"] = int(len(train_scored))
    selected_valid["scored_rows"] = int(len(valid_scored))

    model.save_model(str(output_dir / "lightgbm_model.txt"))
    lgbm_utils.save_feature_state(output_dir / "lightgbm_feature_state.json", CAT_FEATURES, NUM_FEATURES, category_maps)
    pd.DataFrame(train_sweep).to_csv(output_dir / f"lightgbm_level4_threshold_sweep_{args.train_year}.csv", index=False)
    pd.DataFrame(valid_sweep).to_csv(output_dir / f"lightgbm_level4_threshold_sweep_{args.valid_year}.csv", index=False)
    if args.save_score_csvs:
        train_scored.to_csv(output_dir / f"lightgbm_level4_rows_{args.train_year}.csv", index=False)
        valid_scored.to_csv(output_dir / f"lightgbm_level4_rows_{args.valid_year}.csv", index=False)
    write_readable(output_dir, selected_train, selected_valid, args)

    metadata = {
        "run_id": rid,
        "model_type": "lightgbm_oracle_start_level4_specialist_manager",
        "root": str(args.root).upper(),
        ...
        "training_parameters": {
            ...
            "actual_train_rows": int(len(train_for_model)),
        },
        "features": {"cat": CAT_FEATURES, "num": NUM_FEATURES},
        "source_summaries": {
            str(args.train_year): train_summary,
            str(args.valid_year): valid_summary,
        },
        "selected_threshold": float(selected_threshold),
        "selected_train_summary": selected_train,
        "selected_valid_summary": selected_valid,
        "read": "LightGBM Level 4 manager trained from lower-level LightGBM specialist reports.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=lgbm_utils.to_jsonable), encoding="utf-8")
    print(f"selected_lightgbm_level4_threshold={selected_threshold:.6f}", flush=True)
    print(json.dumps({"train_selected": selected_train, "valid_selected": selected_valid}, indent=2, default=lgbm_utils.to_jsonable), flush=True)
    print(f"Saved LightGBM Level 4 manager: {rid}", flush=True)
    return 0
