#!/usr/bin/env python3
"""Train a xgboost Level 1 oracle trend-start specialist on live-grid rows."""
...
def parse_args() -> argparse.Namespace:
    ...
    if parser.run_prefix == "aicw-oracle-start-livegrid-v1":
        parser.run_prefix = "aicw-oracle-start-xgboost-tfspec-v1"
    return parser


def train_model(train: pd.DataFrame, args: argparse.Namespace):
    return xgb_utils.train_binary(train, args, start_model.CAT_FEATURES, start_model.NUM_FEATURES)
...
def main() -> int:
    ...
    print(f"Training xgboost Level 1 rows={len(train):,} positives={int(train['is_oracle_start'].sum()):,}", flush=True)
    ...
    print(f"selected_xgboost_stage1_threshold={selected_threshold:.6f}", flush=True)
    ...
    xgb_utils.save_feature_state(output_dir / "xgboost_feature_state.json", start_model.CAT_FEATURES, start_model.NUM_FEATURES, category_maps)
    ...
    metadata = {
        "run_id": rid,
        "model_type": "xgboost_oracle_start_live_grid_stage1",
        ...
        "read": "xgboost Level 1 specialist trained from live-grid candle rows.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=stage1.to_jsonable), encoding="utf-8")
    print(f"Saved xgboost Level 1 model: {rid}", flush=True)
    return 0
