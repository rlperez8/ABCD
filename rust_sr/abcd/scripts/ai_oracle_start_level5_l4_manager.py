#!/usr/bin/env python3
"""Train a Level 5 manager over the CatBoost, CNN, and LightGBM Level 4 managers.

Level 5 does not read raw candle features as model inputs. It rebuilds the same
live-style candidate grid only so each saved L4 manager can score the same rows,
then learns from those L4 opinions and agreement features.
"""
...
import ai_oracle_start_cnn_level4_livegrid_manager as cnn_l4
...
DEFAULT_CNN_L4_RUN = "aicw-oracle-start-cnn-level4-livegrid-manager-v1-NQ-tr2025-v2026"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-prefix", default="aicw-oracle-start-level5-l4-manager-v1")
    parser.add_argument("--cat-l4-run-id", default=DEFAULT_CAT_L4_RUN)
    parser.add_argument("--cnn-l4-run-id", default=DEFAULT_CNN_L4_RUN)
...
def make_cnn_args(args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        root=str(args.root).upper(),
        roots=str(args.root).upper(),
        timeframes=",".join(parse_timeframes(args.timeframes)),
        oracle_run_id_template="oracle-perfect-trends-tfspec-v1-{timeframe}-2024_2026",
        level1_prefix=cnn_l4.DEFAULT_LEVEL1_PREFIX,
        level1_livegrid_run_id=cnn_l4.DEFAULT_LEVEL1_LIVEGRID_RUN,
        level2_run_id=cnn_l4.DEFAULT_LEVEL2_RUN,
        level3_run_id=cnn_l4.DEFAULT_LEVEL3_RUN,
        train_year=int(args.train_year),
        valid_year=int(args.valid_year),
        match_window_bars=int(args.match_window_bars),
        cooldown_bars=int(args.cooldown_bars),
        ...
        max_manager_train_rows=int(args.max_train_rows),
        max_rows_per_timeframe=0,
...
def maybe_cap_base_rows(frame: pd.DataFrame, args: argparse.Namespace, year: int) -> pd.DataFrame:
    ...
    for timeframe, group in frame.groupby("source_timeframe", sort=False):
        ...
        parts.append(pd.concat([keep_pos, keep_neg], ignore_index=True))
        print(f"{year} {timeframe}: debug-capped base rows {len(group):,} -> {len(parts[-1]):,}", flush=True)
...
def score_cnn_l4(base: pd.DataFrame, year: int, args: argparse.Namespace) -> pd.DataFrame:
    cnn_args = make_cnn_args(args)
    manager_path = wave.ABCD_ROOT / "model_registry" / args.cnn_l4_run_id / "catboost_cnn_level4_livegrid_manager.cbm"
    manager = load_catboost_model(manager_path)
    cnn_l4.visual_cnn.set_seed(int(args.random_seed))
    l1_thresholds = cnn_l4.load_l1_thresholds(cnn_args)
    l1_models, l2_model, l3_model = cnn_l4.load_lower_models(cnn_args)
    ...
    try:
        for timeframe in parse_timeframes(args.timeframes):
            rows = base[base["source_timeframe"].astype(str) == timeframe].copy()
            if rows.empty:
                continue
            print(f"{year} {timeframe}: scoring CNN L4 opinions", flush=True)
            scored = cnn_l4.score_timeframe_rows(
                conn,
                rows,
                int(year),
                timeframe,
            ...
            scored = cnn_l4.add_manager_features(scored, l1_thresholds, l2_threshold, l3_threshold)
            parts.append(scored)
    finally:
        conn.close()
    if not parts:
        raise ValueError(f"No CNN L4 rows scored for {year}")
    reports = pd.concat(parts, ignore_index=True)
    scored = cnn_l4.add_manager_scores(reports, manager)
    out = normalize_keys(scored[KEY_COLS + ["manager_score"]].copy())
    return out.rename(columns={"manager_score": "cnn_l4_score"})
...
def add_l5_features(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for col in ["cat_l4_score", "cnn_l4_score", "light_l4_score"]:
        work[col] = pd.to_numeric(work[col], errors="coerce").fillna(0.0)

    scores = work[["cat_l4_score", "cnn_l4_score", "light_l4_score"]].to_numpy(dtype=float)
    ...
    work["cnn_minus_cat"] = work["cnn_l4_score"] - work["cat_l4_score"]
    work["light_minus_cat"] = work["light_l4_score"] - work["cat_l4_score"]
    work["cnn_minus_light"] = work["cnn_l4_score"] - work["light_l4_score"]

    selected = {
        "cat": selected_threshold(args.cat_l4_run_id, 0.5),
        "cnn": selected_threshold(args.cnn_l4_run_id, 0.5),
        "light": selected_threshold(args.light_l4_run_id, 0.5),
    }
    work["cat_l4_above_selected"] = (work["cat_l4_score"] >= selected["cat"]).astype(int)
    work["cnn_l4_above_selected"] = (work["cnn_l4_score"] >= selected["cnn"]).astype(int)
    work["light_l4_above_selected"] = (work["light_l4_score"] >= selected["light"]).astype(int)
    work["l4_votes_selected"] = work[["cat_l4_above_selected", "cnn_l4_above_selected", "light_l4_above_selected"]].sum(axis=1)
    ...
    work["cnn_l4_above_050"] = (work["cnn_l4_score"] >= 0.5).astype(int)
    work["light_l4_above_050"] = (work["light_l4_score"] >= 0.5).astype(int)
    work["l4_votes_050"] = work[["cat_l4_above_050", "cnn_l4_above_050", "light_l4_above_050"]].sum(axis=1)
    work["all_l4_above_050"] = (work["l4_votes_050"] == 3).astype(int)
    work["any_l4_above_050"] = (work["l4_votes_050"] > 0).astype(int)

    score_cols = ["cat_l4_score", "cnn_l4_score", "light_l4_score"]
    top_idx = scores.argmax(axis=1)
    bottom_idx = scores.argmin(axis=1)
    labels = np.array(["catboost", "cnn", "lightgbm"], dtype=object)
    work["top_l4_block"] = labels[top_idx]
    work["bottom_l4_block"] = labels[bottom_idx]
    return work


def build_l4_opinion_rows(year: int, args: argparse.Namespace) -> tuple[pd.DataFrame, dict[str, Any]]:
    print(f"Building Level 5 base rows year={year}", flush=True)
    ...
    print(f"{year}: scoring CNN L4", flush=True)
    merged = merged.merge(score_cnn_l4(base, int(year), args), on=KEY_COLS, how="inner", validate="one_to_one")
    print(f"{year}: after CNN L4 merge rows={len(merged):,}", flush=True)
...
def cap_train_rows(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if len(positives) >= max_rows:
        keep_pos = positives.sample(n=max_rows, random_state=int(args.random_seed))
        sampled = keep_pos
    else:
...
def event_summary(scored: pd.DataFrame, threshold: float, args: argparse.Namespace, source_summary: dict[str, Any]) -> dict[str, Any]:
    work = scored.copy()
    work["score_num"] = pd.to_numeric(work["level5_score"], errors="coerce").fillna(0.0)
...
def write_readable(output_dir: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with (output_dir / "level5_l4_manager_readable_summary.csv").open("w", newline="", encoding="utf-8") as handle:
...
def write_metadata(
    ...
    metadata = {
        "run_id": rid,
        "model_type": "catboost_oracle_start_level5_l4_manager",
        "root": str(args.root).upper(),
        ...
        "lower_level4_runs": {
            ...
            "cnn_l4_run_id": args.cnn_l4_run_id,
        ...
        "read": "Level 5 manager trained only on CatBoost/CNN/LightGBM Level 4 manager opinions and agreement features.",
    }
...
def main() -> int:
    ...
    print(f"Building Level 5 L4-opinion train rows year={args.train_year}", flush=True)
    train_rows, train_source = build_l4_opinion_rows(int(args.train_year), args)
    print(f"Building Level 5 L4-opinion validation rows year={args.valid_year}", flush=True)
    valid_rows, valid_source = build_l4_opinion_rows(int(args.valid_year), args)

    train_sample = cap_train_rows(train_rows, args)
    print(
        f"Training Level 5 manager rows={len(train_sample):,}/{len(train_rows):,} "
    ...
    model.save_model(str(output_dir / "catboost_oracle_start_level5_l4_manager.cbm"))
    pd.DataFrame(train_sweep).to_csv(output_dir / f"level5_l4_manager_threshold_sweep_{args.train_year}.csv", index=False)
    pd.DataFrame(valid_sweep).to_csv(output_dir / f"level5_l4_manager_threshold_sweep_{args.valid_year}.csv", index=False)
    if args.save_score_csvs:
        train_scored.to_csv(output_dir / f"level5_l4_manager_rows_{args.train_year}.csv", index=False)
        valid_scored.to_csv(output_dir / f"level5_l4_manager_rows_{args.valid_year}.csv", index=False)
    ...
    print(f"selected_level5_l4_threshold={selected_threshold_value:.6f}", flush=True)
    print(json.dumps({"train_selected": selected_train, "valid_selected": selected_valid}, indent=2, default=to_jsonable), flush=True)
    print(f"Saved Level 5 L4 manager: {rid}", flush=True)
    return 0
