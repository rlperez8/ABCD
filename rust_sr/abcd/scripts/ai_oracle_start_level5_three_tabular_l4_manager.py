#!/usr/bin/env python3
"""Train a fast Level 5 overseer over CatBoost, LightGBM, and XGBoost L4 managers.
...
score each row, then trains an overseer on those three L4 opinions only.
"""
...
import ai_oracle_start_xgboost_level4_manager as xgb_l4
...
NUM_FEATURES = [
    ...
    "xgb_l4_above_selected",
    "l4_votes_selected",
    "all_l4_selected",
    "two_or_more_l4_selected",
    "any_l4_selected",
    "cat_l4_above_050",
    "light_l4_above_050",
    "xgb_l4_above_050",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-prefix", default="aicw-oracle-start-level5-three-tabular-l4-manager-v1")
    ...
    parser.add_argument("--opinion-cache-dir", default="", help="Directory for reusable L4 opinion row caches.")
    parser.add_argument("--train-opinion-cache", default="", help="Explicit cached L4 opinion rows for the train year.")
    parser.add_argument("--valid-opinion-cache", default="", help="Explicit cached L4 opinion rows for the validation year.")
    parser.add_argument("--read-opinion-cache", action="store_true", help="Load L4 opinion rows from cache instead of rebuilding/scoring L4.")
    parser.add_argument("--write-opinion-cache", action="store_true", help="Save built L4 opinion rows before Level 5 training.")
    parser.add_argument("--only-build-opinion-cache", action="store_true", help="Build/write L4 opinion caches and exit before Level 5 training.")
    parser.add_argument("--fast-validation-rows", type=int, default=0, help="Debug cap after cache/build. 0 means use all rows.")
...
def default_opinion_cache_dir(args: argparse.Namespace) -> Path:
    ...
    return wave.ABCD_ROOT / "model_registry" / "_l5_opinion_cache"
...
def opinion_cache_digest(args: argparse.Namespace, year: int) -> str:
    payload = {
        ...
        "cat_l4_run_id": args.cat_l4_run_id,
        "light_l4_run_id": args.light_l4_run_id,
        "xgb_l4_run_id": args.xgb_l4_run_id,
        "max_rows_per_timeframe": int(args.max_rows_per_timeframe),
...
def default_opinion_cache_path(args: argparse.Namespace, year: int) -> Path:
    digest = opinion_cache_digest(args, int(year))
    root = str(args.root).upper()
    return default_opinion_cache_dir(args) / f"l4_opinions_{root}_{int(year)}_{digest}.pkl.gz"
...
def write_opinion_cache(frame: pd.DataFrame, summary: dict[str, Any], args: argparse.Namespace, year: int) -> Path:
    ...
    metadata = {
        ...
        "lower_level4_runs": {
            ...
            "xgboost_l4_run_id": args.xgb_l4_run_id,
        },
        "lower_level4_selected_thresholds": {
            ...
            "xgboost": selected_threshold(args.xgb_l4_run_id, 0.5),
        },
    ...
    print(f"Wrote L4 opinion cache year={year} rows={len(frame):,}: {path}", flush=True)
    return path
...
def load_opinion_cache(args: argparse.Namespace, year: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    path = opinion_cache_path_for_year(args, int(year))
    if not path.exists():
        raise FileNotFoundError(f"L4 opinion cache not found: {path}")
    ...
    if metadata_path.exists():
        ...
        summary = {
            ...
            "oracle_starts": int(
                frame.loc[
                    pd.to_numeric(frame["is_oracle_start"], errors="coerce").fillna(0).astype(int) == 1,
    ...
    print(f"Loaded L4 opinion cache year={year} rows={len(frame):,}: {path}", flush=True)
...
def source_summary_for_sample(source_summary: dict[str, Any], frame: pd.DataFrame) -> dict[str, Any]:
    ...
    summary["fast_validation_sample"] = True
    return summary


def maybe_fast_validation_sample(
    frame: pd.DataFrame,
    source_summary: dict[str, Any],
    args: argparse.Namespace,
    year: int,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    cap = int(args.fast_validation_rows)
    if cap <= 0 or len(frame) <= cap:
        return frame, source_summary
    sampled_args = argparse.Namespace(max_train_rows=cap, random_seed=int(args.random_seed) + int(year))
    sampled = cap_train_rows(frame, sampled_args)
    sampled_summary = source_summary_for_sample(source_summary, sampled)
    print(f"{year}: fast-validation sampled opinion rows {len(frame):,} -> {len(sampled):,}", flush=True)
...
def make_base_build_args(args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        run_prefix="level5-three-tabular-row-build",
...
def make_xgb_args(args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        train_year=int(args.train_year),
        valid_year=int(args.valid_year),
        level1_prefix=xgb_l4.DEFAULT_LEVEL1_PREFIX,
        level2_run_id=xgb_l4.DEFAULT_LEVEL2_RUN,
        level3_run_id=xgb_l4.DEFAULT_LEVEL3_RUN,
        random_seed=int(args.random_seed),
        cooldown_bars=int(args.cooldown_bars),
        match_window_bars=int(args.match_window_bars),
        thresholds=args.thresholds,
...
def score_xgb_l4(base: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    model_dir = wave.ABCD_ROOT / "model_registry" / args.xgb_l4_run_id
    model, _cat_features, _num_features, category_maps = xgb_utils.load_model(model_dir)
    reports = xgb_l4.score_lower_levels(base, make_xgb_args(args))
    scored = xgb_l4.add_manager_scores(reports, (model, category_maps))
    out = normalize_keys(scored[KEY_COLS + ["manager_score"]].copy())
    return out.rename(columns={"manager_score": "xgb_l4_score"})
...
def add_l5_features(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for col in ["cat_l4_score", "light_l4_score", "xgb_l4_score"]:
        work[col] = pd.to_numeric(work[col], errors="coerce").fillna(0.0)
    scores = work[["cat_l4_score", "light_l4_score", "xgb_l4_score"]].to_numpy(dtype=float)
    ...
    work["xgb_minus_cat"] = work["xgb_l4_score"] - work["cat_l4_score"]
    work["xgb_minus_light"] = work["xgb_l4_score"] - work["light_l4_score"]

    cat_selected = selected_threshold(args.cat_l4_run_id, 0.5)
    light_selected = selected_threshold(args.light_l4_run_id, 0.5)
    xgb_selected = selected_threshold(args.xgb_l4_run_id, 0.5)
    ...
    work["xgb_l4_above_selected"] = (work["xgb_l4_score"] >= xgb_selected).astype(int)
    work["l4_votes_selected"] = work[["cat_l4_above_selected", "light_l4_above_selected", "xgb_l4_above_selected"]].sum(axis=1)
    work["all_l4_selected"] = (work["l4_votes_selected"] == 3).astype(int)
    work["two_or_more_l4_selected"] = (work["l4_votes_selected"] >= 2).astype(int)
    ...
    work["xgb_l4_above_050"] = (work["xgb_l4_score"] >= 0.5).astype(int)
    work["l4_votes_050"] = work[["cat_l4_above_050", "light_l4_above_050", "xgb_l4_above_050"]].sum(axis=1)
    work["all_l4_above_050"] = (work["l4_votes_050"] == 3).astype(int)
    work["two_or_more_l4_050"] = (work["l4_votes_050"] >= 2).astype(int)
    work["any_l4_above_050"] = (work["l4_votes_050"] > 0).astype(int)
    top_indices = np.argmax(scores, axis=1)
    work["top_l4_block"] = np.take(np.asarray(["catboost", "lightgbm", "xgboost"], dtype=object), top_indices)
    return work


def build_l4_opinion_rows(year: int, args: argparse.Namespace) -> tuple[pd.DataFrame, dict[str, Any]]:
    print(f"Building three-opinion tabular Level 5 base rows year={year}", flush=True)
    ...
    print(f"{year}: scoring XGBoost L4", flush=True)
    merged = merged.merge(score_xgb_l4(base, args), on=KEY_COLS, how="inner", validate="one_to_one")
    print(f"{year}: after XGBoost L4 merge rows={len(merged):,}", flush=True)
...
def cap_train_rows(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if positives.empty or negatives.empty:
        return frame.sample(n=max_rows, random_state=int(args.random_seed)).reset_index(drop=True)
    ...
    if keep_pos_n <= 0:
        keep_pos_n = 1
        keep_neg_n = max(1, max_rows - keep_pos_n)
    ...
    sampled = pd.concat([keep_pos, keep_neg], ignore_index=True)
...
def write_readable(output_dir: Path, rows: list[dict[str, Any]]) -> None:
    with (output_dir / "level5_three_tabular_l4_readable_summary.csv").open("w", newline="", encoding="utf-8") as handle:
...
def write_metadata(
    ...
    metadata = {
        "run_id": rid,
        "model_type": "catboost_oracle_start_level5_three_tabular_l4_manager",
        "root": str(args.root).upper(),
        "timeframes": parse_timeframes(args.timeframes),
        "train_year": int(args.train_year),
        "valid_year": int(args.valid_year),
        ...
        "training_parameters": {
            ...
            "fast_validation_rows": int(args.fast_validation_rows),
        ...
        "read": "Fast Level 5 trained only on CatBoost, LightGBM, and XGBoost L4 manager opinions.",
    }
...
def main() -> int:
    ...
    if args.read_opinion_cache:
        ...
        print(f"Building three-opinion tabular Level 5 train rows year={args.train_year}", flush=True)
        train_rows, train_source = build_l4_opinion_rows(int(args.train_year), args)
        print(f"Building three-opinion tabular Level 5 validation rows year={args.valid_year}", flush=True)
    ...
    if args.only_build_opinion_cache:
        print("Built L4 opinion caches only; skipping Level 5 training.", flush=True)
        return 0

    train_rows, train_source = maybe_fast_validation_sample(train_rows, train_source, args, int(args.train_year))
    valid_rows, valid_source = maybe_fast_validation_sample(valid_rows, valid_source, args, int(args.valid_year))

    train_sample = cap_train_rows(train_rows, args)
    print(
        f"Training three-opinion tabular Level 5 rows={len(train_sample):,}/{len(train_rows):,} "
    ...
    model.save_model(str(output_dir / "catboost_oracle_start_level5_three_tabular_l4_manager.cbm"))
    pd.DataFrame(train_sweep).to_csv(output_dir / f"level5_three_tabular_l4_threshold_sweep_{args.train_year}.csv", index=False)
    pd.DataFrame(valid_sweep).to_csv(output_dir / f"level5_three_tabular_l4_threshold_sweep_{args.valid_year}.csv", index=False)
    if args.save_score_csvs:
        train_scored.to_csv(output_dir / f"level5_three_tabular_l4_rows_{args.train_year}.csv", index=False)
        valid_scored.to_csv(output_dir / f"level5_three_tabular_l4_rows_{args.valid_year}.csv", index=False)
    ...
    print(f"selected_level5_three_tabular_l4_threshold={selected_threshold_value:.6f}", flush=True)
    print(json.dumps({"train_selected": selected_train, "valid_selected": selected_valid}, indent=2, default=to_jsonable), flush=True)
    print(f"Saved three-opinion tabular Level 5 manager: {rid}", flush=True)
    return 0
