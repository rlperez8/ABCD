#!/usr/bin/env python3
"""Train a LightGBM Level 3 global all-root/all-timeframe specialist."""
...
def parse_args() -> argparse.Namespace:
    ...
    if args.run_prefix == "aicw-oracle-start-global-alltf-v1":
        args.run_prefix = "aicw-oracle-start-lightgbm-global-alltf-v1"
    return args
...
def main() -> int:
    ...
    print(f"Training LightGBM Level 3 rows={len(train):,} positives={int(train['is_oracle_start'].sum()):,}", flush=True)
    ...
    print(f"selected_lightgbm_global_alltf_stage1_threshold={selected_threshold:.6f}", flush=True)
    ...
    metadata = {
        "run_id": rid,
        "model_type": "lightgbm_oracle_start_global_alltf_stage1",
        ...
        "read": "LightGBM Level 3 specialist trained across selected roots and selected timeframes.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=root_alltf.to_jsonable), encoding="utf-8")
    print(f"Saved LightGBM Level 3 model: {rid}", flush=True)
    return 0
