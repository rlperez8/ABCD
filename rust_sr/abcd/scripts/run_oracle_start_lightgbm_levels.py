#!/usr/bin/env python3
"""Run the LightGBM oracle-start level system sequentially and resumably."""
...
import ai_oracle_start_lightgbm_level4_manager as l4
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--l1-prefix", default="aicw-oracle-start-lightgbm-tfspec-v1")
    parser.add_argument("--l2-prefix", default="aicw-oracle-start-lightgbm-root-alltf-v1")
    parser.add_argument("--l3-prefix", default="aicw-oracle-start-lightgbm-global-alltf-v1")
    parser.add_argument("--l4-prefix", default="aicw-oracle-start-lightgbm-level4-manager-v1")
...
def main() -> int:
    ...
    log_dir = wave.ABCD_ROOT / "logs" / "oracle_start_lightgbm_levels"
    log_dir.mkdir(parents=True, exist_ok=True)
    summary_path = log_dir / f"{args.l4_prefix}-{args.root}-summary.json"
    started = time.time()
    rows: list[dict[str, Any]] = []

    l1_script = SCRIPT_DIR / "ai_oracle_start_lightgbm_stage1_live_grid_model.py"
    ...
    rows.append(
        run_step(
            ...
            [
                sys.executable,
                str(SCRIPT_DIR / "ai_oracle_start_lightgbm_root_alltf_model.py"),
                "--root",
                args.root,
                "--timeframes",
                args.timeframes,
    ...
    rows.append(
        run_step(
            ...
            [
                sys.executable,
                str(SCRIPT_DIR / "ai_oracle_start_lightgbm_global_alltf_model.py"),
                "--roots",
                roots_for_l3,
                "--timeframes",
                args.timeframes,
    ...
    rows.append(
        run_step(
            ...
            [
                sys.executable,
                str(SCRIPT_DIR / "ai_oracle_start_lightgbm_level4_manager.py"),
                "--root",
                args.root,
                "--timeframes",
                args.timeframes,
