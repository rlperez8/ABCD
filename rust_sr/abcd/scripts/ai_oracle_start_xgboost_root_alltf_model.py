#!/usr/bin/env python3
"""Train a xgboost Level 2 root/all-timeframe oracle trend-start specialist."""
...
def parse_args() -> argparse.Namespace:
    ...
    if args.run_prefix == "aicw-oracle-start-root-alltf-v1":
        args.run_prefix = "aicw-oracle-start-xgboost-root-alltf-v1"
    return args
...
def main() -> int:
    ...
    print(f"Training xgboost Level 2 rows={len(train):,} positives={int(train['is_oracle_start'].sum()):,}", flush=True)
    ...
    print(f"selected_xgboost_root_alltf_stage1_threshold={selected_threshold:.6f}", flush=True)
    ...
    metadata = {
        "run_id": rid,
        "model_type": "xgboost_oracle_start_root_alltf_stage1",
        ...
        "read": "xgboost Level 2 specialist trained on one root across selected timeframes.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=root_alltf.to_jsonable), encoding="utf-8")
    print(f"Saved xgboost Level 2 model: {rid}", flush=True)
    return 0
