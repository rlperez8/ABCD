#!/usr/bin/env python3
"""Train a xgboost Level 3 global all-root/all-timeframe specialist."""
...
def parse_args() -> argparse.Namespace:
    args = global_alltf.parse_args()
    if args.run_prefix == "aicw-oracle-start-global-alltf-v1":
        args.run_prefix = "aicw-oracle-start-xgboost-global-alltf-v1"
    return args
...
def add_scores(frame: pd.DataFrame, model_bundle) -> pd.DataFrame:
    ...
    scored = frame.copy()
    scored["oracle_start_score"] = xgb_utils.predict_scores(
        scored,
        model,
...
def main() -> int:
    ...
    try:
        ...
        train_summaries = {}
        for year in global_alltf.parse_years(args.train_years):
            frame, summary = global_alltf.build_year_rows(conn, int(year), args, rng, train_sample=True)
            train_parts.append(frame)
            train_summaries[str(year)] = summary
        threshold, threshold_summary = global_alltf.build_year_rows(conn, int(args.threshold_year), args, rng, train_sample=False)
        valid, valid_summary = global_alltf.build_year_rows(conn, int(args.valid_year), args, rng, train_sample=False)
    finally:
        conn.close()

    train = pd.concat(train_parts, ignore_index=True).sample(frac=1.0, random_state=int(args.random_seed)).reset_index(drop=True)
    print(f"Training xgboost Level 3 rows={len(train):,} positives={int(train['is_oracle_start'].sum()):,}", flush=True)
    model, category_maps = xgb_utils.train_binary(train, args, root_alltf.CAT_FEATURES, root_alltf.NUM_FEATURES)
    ...
    print(f"selected_xgboost_global_alltf_stage1_threshold={selected_threshold:.6f}", flush=True)
    ...
    xgb_utils.save_feature_state(output_dir / "xgboost_feature_state.json", root_alltf.CAT_FEATURES, root_alltf.NUM_FEATURES, category_maps)
    ...
    metadata = {
        "run_id": rid,
        "model_type": "xgboost_oracle_start_global_alltf_stage1",
        "oracle_run_id_template": args.oracle_run_id_template,
        "roots": global_alltf.parse_roots(args.roots),
        "timeframes": global_alltf.parse_timeframes(args.timeframes),
        "train_years": global_alltf.parse_years(args.train_years),
        ...
        "read": "xgboost Level 3 specialist trained across selected roots and selected timeframes.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=root_alltf.to_jsonable), encoding="utf-8")
    print(f"Saved xgboost Level 3 model: {rid}", flush=True)
    return 0
