#!/usr/bin/env python3
"""Train a xgboost Level 4 manager over xgboost L1/L2/L3 specialist scores."""
...
DEFAULT_LEVEL1_PREFIX = "aicw-oracle-start-xgboost-tfspec-v1"
DEFAULT_LEVEL2_RUN = "aicw-oracle-start-xgboost-root-alltf-v1-NQ-alltf-tr2024-v2026"
DEFAULT_LEVEL3_RUN = "aicw-oracle-start-xgboost-global-alltf-v1-NQ-alltf-tr2024-v2026"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-prefix", default="aicw-oracle-start-xgboost-level4-manager-v1")
...
def make_build_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        run_prefix="xgboost-level4-row-build",
...
def load_xgboost_model(run_id_value: str):
    return xgb_utils.load_model(wave.ABCD_ROOT / "model_registry" / run_id_value)


def score_with_xgboost(frame: pd.DataFrame, run_id_value: str) -> np.ndarray:
    model, cat_features, num_features, category_maps = load_xgboost_model(run_id_value)
    return xgb_utils.predict_scores(frame, model, cat_features, num_features, category_maps)
...
def score_lower_levels(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for timeframe in parse_timeframes(args.timeframes):
        ...
        if part.empty:
            continue
        part["l1_score"] = score_with_xgboost(part, level1_run_id(args, timeframe))
    ...
    work["l2_score"] = score_with_xgboost(work, args.level2_run_id)
    work["l3_score"] = score_with_xgboost(work, args.level3_run_id)
...
def add_manager_scores(frame: pd.DataFrame, model_bundle) -> pd.DataFrame:
    ...
    scored = frame.copy()
    scored["manager_score"] = xgb_utils.predict_scores(scored, model, CAT_FEATURES, NUM_FEATURES, category_maps)
    return scored
...
def write_readable(output_dir: Path, train_summary: dict[str, Any], valid_summary: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    with (output_dir / "xgboost_level4_readable_summary.csv").open("w", newline="", encoding="utf-8") as handle:
...
def main() -> int:
    ...
    print(f"Building xgboost lower-level reports for train year={args.train_year}", flush=True)
    train_base, train_summary = build_base_rows(int(args.train_year), args)
    train_reports = score_lower_levels(train_base, args)
    print(f"Building xgboost lower-level reports for validation year={args.valid_year}", flush=True)
    ...
    print(
        f"Training xgboost Level 4 manager rows={len(train_for_model):,}/{len(train_reports):,} "
        f"positives={int(train_for_model['is_oracle_start'].sum()):,}",
        flush=True,
    )
    model, category_maps = xgb_utils.train_binary(train_for_model, args, CAT_FEATURES, NUM_FEATURES)
    ...
    xgb_utils.save_feature_state(output_dir / "xgboost_feature_state.json", CAT_FEATURES, NUM_FEATURES, category_maps)
    pd.DataFrame(train_sweep).to_csv(output_dir / f"xgboost_level4_threshold_sweep_{args.train_year}.csv", index=False)
    pd.DataFrame(valid_sweep).to_csv(output_dir / f"xgboost_level4_threshold_sweep_{args.valid_year}.csv", index=False)
    if args.save_score_csvs:
        train_scored.to_csv(output_dir / f"xgboost_level4_rows_{args.train_year}.csv", index=False)
        valid_scored.to_csv(output_dir / f"xgboost_level4_rows_{args.valid_year}.csv", index=False)
    write_readable(output_dir, selected_train, selected_valid, args)

    metadata = {
        "run_id": rid,
        "model_type": "xgboost_oracle_start_level4_specialist_manager",
        "root": str(args.root).upper(),
        ...
        "read": "xgboost Level 4 manager trained from lower-level xgboost specialist reports.",
    }
    (output_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, default=xgb_utils.to_jsonable), encoding="utf-8")
    print(f"selected_xgboost_level4_threshold={selected_threshold:.6f}", flush=True)
    print(json.dumps({"train_selected": selected_train, "valid_selected": selected_valid}, indent=2, default=xgb_utils.to_jsonable), flush=True)
    print(f"Saved xgboost Level 4 manager: {rid}", flush=True)
    return 0
