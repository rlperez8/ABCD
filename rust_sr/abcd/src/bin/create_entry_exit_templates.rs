...
fn parse_args() -> Result<Args, Box<dyn std::error::Error>> {
    ...
    let period_year = if start_year == end_year { start_year } else { 0 };
...
async fn ensure_entry_exit_template_indexes(pool: &MySqlPool) -> Result<(), sqlx::Error> {
    let index_name = "idx_entry_exit_template_results_run_template_stats";
    ...
    if exists == 0 {
        println!("Adding index {index_name} for full-run Entry/Exit template stats...");
        sqlx::query(
            r#"
            ALTER TABLE entry_exit_template_results
            ADD INDEX idx_entry_exit_template_results_run_template_stats
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!(
        "Stored creator run {run_id}: scanned={}, templates_created={}, existing_template_passes={}, failed_to_create={}, result_rows={}, ui_stats_rows={}, elapsed={:.2}s",
        patterns.len(),
        templates.len(),
        existing_template_passes,
        failed_to_create,
