...
// === Enum ===/// Enum for detected reversal patterns
...
pub struct Trade {
    ...
    pub month: i64,
    pub day: i64,
    pub reversalType: ReversalPattern
...
impl ABCD {
    ...
    pub fn detect_patterns(candles: &Vec<Candle>, symbol: String) -> mysql::Result<Vec<PatternABCD>>{
            ...
            for pattern in &mut pattern_abc {
                ...
                if prev1.low <= pattern.c.min_max && prev1.low < pattern.b.low {
                    if Self::check_for_pivot(current, prev1, prev2, PivotType::Low) {
                        ...
                        fn detect_pattern(c1: &Candle, c2: &Candle, c3: &Candle) -> ReversalPattern {
                        
                            if is_morning_star(c1, c2, c3) {
                                ReversalPattern::MorningStar
                            } else if is_evening_star(c1, c2, c3) {
                                ReversalPattern::EveningStar
                            } else {
